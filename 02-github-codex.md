...
2026年5月時点では、OpenAIの公式ドキュメントでは多くのコード生成タスクの標準として `gpt-5.5` が案内されています。Codex CLIのモデル選択画面では、APIキーの設定や利用できるモデルによって、`gpt-5.5`、`gpt-5.4`、`gpt-5.3-codex` など表示される候補が変わることがあります。
